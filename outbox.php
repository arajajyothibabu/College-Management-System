...
<table width="100%">
<?php 
$query = mysql_query("select * from letter where $id = '{$uid}' order by sno DESC");
if(!$query){
	...
	while($q = mysql_fetch_array($query)){
		...
		echo '<tr><td><a href="viewletter.php?link=outbox&&id='. $q['sno'] .'" target="main"><fieldset id="'.$color.'"><table width="100%"><tr><td width="5%">'. $i .'</td><td>&nbsp</td><td><strong>'.$q["letterto"].'</strong></td><td>&nbsp</td><td><marquee behavior="scroll" dir="ltr">'.$q['subject'].'</marquee></td><td>&nbsp</td><td width="8%"> '.$q["dept"].'</td><td>&nbsp</td><td width="15%">'.$q["date"].'</td></tr></table></fieldset></a></td></tr>';
