...
<table width="100%" id="structure">
	<tr>
       <td id="page"><fieldset style="border-style:ridge"><legend align="center">Academic Search</legend>
       <!--<h2 style="text-decoration:underline" align="center">Faculty Academics</h2>-->
       <table width="100%"><form action="student.php?link=" method="post" name="f"><tr><td>
...
if(isset($_POST['search']))
{
 if(!empty($details))
    ...
    echo '<tr><td><table width="100%" height="100%" border="1">
			<tr>
			  <td width="90%" align="center"><table width="100%" border="0">
				<tr>
				  <td width="" align="center"><h3><strong>'.$details['username'].'</strong></h3></td>
				</tr>
				
				<tr>
				  <td align="center"><strong>'.$details['dept'].', Section-'.$details['section'].'</strong></td>
				</tr>
				<tr><td><h2></h2></td></tr>
				<tr>
				  <td align="center"><strong>'.$details['email'].'</strong></td>
				</tr>
				<tr><td><h2></h2></td></tr>
				<tr>
				  <td align="center"><strong>'.$details['address'].'</strong></td>
				</tr>
			  </table></td>
			  <td width="10%" align="center"><img src="showimage.php?id='.$_SESSION['user_id'].'&view=jb" alt="Profile Photo" width="140" height="140"/></td>
			</tr>
	...
	if(notnull($semester))
	{
						$queryresult = mysql_query("select * from marks where sid = '{$_SESSION['user_id']}' and semester = '{$semester}'");
...
{			//code for all semesters.
	...
	$countsem = mysql_query("select max(semester) from marks where sid = '{$_SESSION['user_id']}'");
	$count = mysql_fetch_array($countsem);
	for($i = 1;$i<=$count[0];$i++){
	//semester 1
						$queryresult = mysql_query("select * from marks where sid = '{$_SESSION['user_id']}' and semester = '{$i}'");
						confirm_query($queryresult);
								if (!$queryresult) {
									// username/password authenticated
									// and only 1 match				
